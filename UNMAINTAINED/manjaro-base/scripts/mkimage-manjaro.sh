#!/usr/bin/env bash
# Generate a minimal filesystem for manjaro and load it into the local
# docker as "manjaro"
# requires root
set -e
...
case "$(uname -m)" in
	armv*)
		...
		;;
	*)
		PACMAN_CONF='./mkimage-manjaro-pacman.conf'
		PACMAN_MIRRORLIST='Server = http://lug.mtu.edu/manjaro/stable/$repo/$arch'
		PACMAN_EXTRA_PKGS='archlinux-keyring manjaro-keyring'
		EXPECT_TIMEOUT=60
		ARCH_KEYRING='archlinux manjaro'
		DOCKER_IMAGE_NAME=manjaro
		;;
esac
